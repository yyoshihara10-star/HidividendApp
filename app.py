...
st.set_page_config(page_title="プライム高配当株・完全完走版", layout="wide")
st.title("高配当株スクリーニング (リトライ強化・エラーガード版)")
...
def get_info_with_resilience(symbol, max_retries=3):
    """指数バックオフによる再試行とNoneチェックを徹底"""
    stock = yf.Ticker(symbol)
    for i in range(max_retries):
        try:
            # yfinanceの内部キャッシュをクリアして新鮮なデータを要求
            info = stock.info
            if info and isinstance(info, dict) and 'shortName' in info:
                return info, stock
        except Exception:
            wait_time = (i + 1) * 3 # 3s, 6s, 9s ...
            time.sleep(wait_time)
            continue
    return None, None

def analyze_stock_final_ver(symbol, industry):
    info, stock = get_info_with_resilience(symbol)
    
    # ここでNoneチェックを厳格に行う（エラーの主因をブロック）
...
if st.button("🚀 再試行リトライ・スキャン開始", type="primary"):
    ...
    for idx, industry in enumerate(all_industries):
        ...
        for _, row in sector_members.iterrows():
            res = analyze_stock_final_ver(f"{row['コード']}.T", industry)
            if res:
...
if not st.session_state['result_df'].empty:
    ...
    st.download_button("📥 完全データ版CSV", df_show.to_csv(index=False).encode('utf-8-sig'), "prime_final_scan.csv")
